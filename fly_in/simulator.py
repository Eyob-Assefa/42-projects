...
class Simulator:
    """Executes the drone movements turn-by-turn and enforces simulation rules."""
    ...
    def __init__(self, graph: Graph, drones: list[Drone]) -> None:
        ...
        # Track dynamic occupancy of zones separately from the static Graph
    ...
    def run(self) -> None:
        ...
        while not self._all_drones_finished():
            ...
            # Print the combined movements for this turn, space-separated
            if turn_output:
    ...
    def _execute_turn(self) -> list[str]:
        """Executes a discrete turn using a two-phase resolution mechanism."""
        turn_logs: list[str] = []
        ...
        for drone in self.drones:
            ...
            if drone.turns_in_transit > 0:
                ...
                if drone.turns_in_transit == 0:
                    # Transit complete. Drone is ready to arrive at the zone this turn.
                    intended_moves[drone] = (next_zone, None)
                else:
                    # The rules mandate printing the connection name for mid-air drones
                    turn_logs.append(f"D{drone.drone_id}-transit")
                continue

            # Handle standard zone departures
            if drone.current_location and drone.current_location != next_zone:
                
                # Rule: Drones moving out of a zone free up capacity for that same turn
                ...
                if next_zone.zone_type == "restricted":
                    # Construct a connection string for the output format
                    connection_name = f"{drone.current_location.name}-{next_zone.name}"
                    intended_moves[drone] = (next_zone, connection_name)
                else:
                    intended_moves[drone] = (next_zone, None)
        ...
        for drone, (target_zone, connection_name) in intended_moves.items():
            ...
            # Scenario B: Arriving at a zone (Normal/Priority or Turn 2 of Restricted)
            is_end_hub = (target_zone == self.graph.end_hub)
            has_space = (self.zone_occupancy[target_zone.name] < target_zone.max_drones)

            if is_end_hub or has_space:
                drone.move(target_zone, is_restricted=False)
                
                # The start/end hubs ignore standard capacity limitations
                if not is_end_hub:
                    self.zone_occupancy[target_zone.name] += 1
                    
                turn_logs.append(f"D{drone.drone_id}-{target_zone.name}")
            else:
                # If this hits, the pathfinding algorithm supplied an overlapping route
                raise RuntimeError(
                    f"Collision error: Zone {target_zone.name} exceeded max capacity."
                )
